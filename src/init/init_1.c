...
/*   Updated: 2024/05/21 19:31:35 by edoardo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_sprite(t_sprite *texture, t_game *game)
{
	...
	if (texture->img == NULL)
		error("error on init_sprite, mlx_xpm_file_to_image() failed", game);
	texture->addr = mlx_get_data_addr(texture->img, &texture->bpp,
			&texture->line_len, &texture->endian);
	texture->text_value = (int *)mlx_get_data_addr(texture->img, &texture->bpp,
			&texture->line_len, &texture->endian);
	if (!texture->addr)
		error("error on init_sprite, mlx_get_data_addr() failed", game);
}
